...
# Production-safe configuration via environment variables.
# Local development still works with a local MySQL database when DATABASE_URL is unset.
DB_URI = os.environ.get("DATABASE_URL") or os.environ.get("MYSQL_URL") or "mysql+pymysql://root:password@localhost/stegano_db"
...
# Set STORAGE_DIR=/data on Railway when a persistent volume is mounted at /data.
STORAGE_DIR = os.environ.get("STORAGE_DIR") or os.environ.get("RAILWAY_VOLUME_MOUNT_PATH") or "."
UPLOAD_FOLDER = os.path.join(STORAGE_DIR, "uploads")
STEGO_FOLDER = os.path.join(STORAGE_DIR, "stego_store")

# Public URL used in QR codes. In production set this to the HTTPS Railway domain.
...
app = Flask(__name__)
# Railway/proxies terminate HTTPS before forwarding to Gunicorn.
...
def embed_image():
    if request.method == "POST":
        ...
        # Build the QR URL from the public application URL instead of the
        # machine's LAN IP. This allows phones on different networks to scan it.
